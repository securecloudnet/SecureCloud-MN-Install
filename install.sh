...
TARBALLURL="https://github.com/securecloudnet/SecureCloud/releases/download/2.0.0/securecloud-2.0.0-x86_64-linux-gnu.tar.gz"
TARBALLNAME="securecloud-2.0.0-x86_64-linux-gnu.tar.gz"
BOOTSTRAPURL=""
BOOTSTRAPARCHIVE=""
BWKVERSION="1.0.0"
...
echo "
 ...
 | the SCN Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "securecloud"' && echo ""
sleep 1
...
# Create .securecloud directory
mkdir $USERHOME/.securecloud
...
# Create securecloud.conf
touch $USERHOME/.securecloud/securecloud.conf
cat > $USERHOME/.securecloud/securecloud.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
cat > /etc/systemd/system/securecloud.service << EOL
[Unit]
Description=securecloudd
After=network.target
[Service]
Type=forking
User=${USER}
WorkingDirectory=${USERHOME}
ExecStart=/usr/local/bin/securecloudd -conf=${USERHOME}/.securecloud/securecloud.conf -datadir=${USERHOME}/.securecloud
ExecStop=/usr/local/bin/securecloud-cli -conf=${USERHOME}/.securecloud/securecloud.conf -datadir=${USERHOME}/.securecloud stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
sudo systemctl enable securecloud.service
sudo systemctl start securecloud.service
